...
FAVORITES="${FAVORITES:-brave,firefox,code,terminal,files,settings}"
...
# ── Browser & Favorites Detection ──────────────────────────────────────────

# Search for a .desktop file by checking common locations.
# Returns the full .desktop filename (e.g. "firefox.desktop") or empty.
find_desktop_file() {
    local -a patterns=("$@")
    local -a search_dirs=(
    ...
    for pattern in "${patterns[@]}"; do
        for dir in "${search_dirs[@]}"; do
            if [[ -d "$dir" ]]; then
                local found
                # Look for exact match first, then glob
                if [[ -f "${dir}/${pattern}" ]]; then
                    printf "%s" "$pattern"
                    return 0
                fi
                # Try glob match (e.g. "brave*.desktop")
                found=$(find "$dir" -maxdepth 1 -name "$pattern" -print -quit 2>/dev/null)
                if [[ -n "$found" ]]; then
                    basename "$found"
                    return 0
                fi
...
# Detect the best available browser .desktop file
detect_browser() {
    local result

    # Brave — multiple known .desktop names
    result=$(find_desktop_file "brave-browser.desktop" "brave_brave.desktop" "com.brave.Browser.desktop") && {
        printf "%s" "$result"; return 0
    }

    # Firefox — snap, flatpak, deb
    result=$(find_desktop_file "firefox.desktop" "firefox_firefox.desktop" "org.mozilla.firefox.desktop") && {
        printf "%s" "$result"; return 0
    }

    # Google Chrome
    result=$(find_desktop_file "google-chrome.desktop" "google-chrome-stable.desktop" "com.google.Chrome.desktop") && {
        printf "%s" "$result"; return 0
    }

    # Chromium
    result=$(find_desktop_file "chromium-browser.desktop" "chromium.desktop" "org.chromium.Chromium.desktop") && {
        printf "%s" "$result"; return 0
    }

    # Fallback — any browser-like .desktop
    local -a search_dirs=(
        "/usr/share/applications"
        "/var/lib/snapd/desktop/applications"
    )
    for dir in "${search_dirs[@]}"; do
        if [[ -d "$dir" ]]; then
            local found
            found=$(grep -rlm1 "WebBrowser" "$dir"/*.desktop 2>/dev/null | head -1)
            if [[ -n "$found" ]]; then
                basename "$found"
                return 0
            fi
...
# Detect a .desktop file for Visual Studio Code / Code OSS
detect_code_editor() {
    local result
    result=$(find_desktop_file \
        "code.desktop" \
        "code_code.desktop" \
...
# Detect terminal emulator .desktop file
detect_terminal() {
    local result
    result=$(find_desktop_file \
...
# Detect file manager .desktop file
detect_files() {
    local result
    result=$(find_desktop_file \
...
# Detect GNOME Settings .desktop file
detect_settings() {
    local result
    result=$(find_desktop_file \
...
# Build favorite-apps array dynamically from config FAVORITES string.
# Only includes apps whose .desktop files actually exist.
# Returns a gsettings-compatible array string like: ['app1.desktop', 'app2.desktop']
build_favorites() {
    local favorites_str="${FAVORITES:-brave,firefox,code,terminal,files,settings}"
    local -a result=()

    IFS=',' read -ra keywords <<< "$favorites_str"
    ...
    for keyword in "${keywords[@]}"; do
        ...
        case "$keyword" in
            brave|firefox|chrome|chromium)
                desktop=$(detect_browser) || true
                # If we already added a browser, skip duplicates
                if [[ -n "$desktop" ]]; then
                    local already_added=false
                    for existing in "${result[@]+"${result[@]}"}"; do
                        if [[ "$existing" == "$desktop" ]]; then
                            already_added=true
                            break
                        fi
                    done
                ...
                ;;
            code|vscode|codium)
                desktop=$(detect_code_editor) || true
                ;;
            terminal)
                desktop=$(detect_terminal) || true
                ;;
            files|nautilus)
                desktop=$(detect_files) || true
                ;;
            settings)
                desktop=$(detect_settings) || true
                ;;
            text-editor)
                desktop=$(find_desktop_file "org.gnome.TextEditor.desktop" "gedit.desktop") || true
                ;;
            *)
                # Try as a literal .desktop filename
                if [[ "$keyword" == *.desktop ]]; then
                    desktop=$(find_desktop_file "$keyword") || true
                else
                    desktop=$(find_desktop_file "${keyword}.desktop") || true
                fi
                ;;
        esac
        ...
        if [[ -n "$desktop" ]]; then
            ...
            log "[WARN] Favorite '$keyword': no matching .desktop file found — skipping"
        fi
    done
